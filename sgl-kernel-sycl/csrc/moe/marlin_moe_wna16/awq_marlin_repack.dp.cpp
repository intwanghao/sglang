...
void awq_marlin_repack_kernel(
  ...
  auto fetch_to_shared = [&](int pipe, int k_tile_id, int n_tile_id, uint8_t *dpct_local) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
  ...
  /*
  DPCT1110:642: The total declared local variable size in device function operator() exceeds 128 bytes and may cause
  ...
  */
  auto repack_tile = [&](int pipe, int k_tile_id, int n_tile_id, uint8_t* dpct_local) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    if (n_tile_id >= n_tiles) {
      return;
    }
...
#pragma unroll
  ...
  auto start_pipes = [&](int k_tile_id, int n_tile_id, uint8_t *dpct_local) {
#pragma unroll
    for (int pipe = 0; pipe < repack_stages - 1; pipe++) {
      fetch_to_shared(pipe, k_tile_id, n_tile_id + pipe, dpct_local);
    }
...
#pragma unroll
  for (int k_tile_id = start_k_tile; k_tile_id < finish_k_tile; k_tile_id++) {
    ...
    start_pipes(k_tile_id, n_tile_id, dpct_local);

    while (n_tile_id < n_tiles) {
#pragma unroll
      for (int pipe = 0; pipe < repack_stages; pipe++) {
        fetch_to_shared(
            (pipe + repack_stages - 1) % repack_stages, k_tile_id, n_tile_id + pipe + repack_stages - 1, dpct_local);
        repack_tile(pipe, k_tile_id, n_tile_id + pipe, dpct_local);
        wait_for_stage();
      }
...
/*
DPCT1026:798: The call to cudaFuncSetAttribute was removed because SYCL currently does not support corresponding
setting.
*/
#define CALL_IF(NUM_BITS)                                                                                           \
    ...
    {                                                                                                               \
      ...
      stream->submit([&](sycl::handler& cgh) {                                                                      \
        ...
        auto out_ptr_ct1 = out_ptr;                                                                                 \
        int size_k_ct2 = size_k;                                                                                    \
        int size_n_ct3 = size_n;                                                                                    \
                                                                                                                    \
...
torch::Tensor awq_marlin_repack(torch::Tensor& b_q_weight, int64_t size_k, int64_t size_n, int64_t num_bits) {
  ...
  /*
  DPCT1019:797: local_mem_size in SYCL is not a complete equivalent of cudaDevAttrMaxSharedMemoryPerBlockOptin in CUDA.
