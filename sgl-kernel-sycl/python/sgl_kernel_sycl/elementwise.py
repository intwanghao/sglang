...
def rmsnorm(
    ...
    torch.ops.sgl_kernel_sycl.rmsnorm.default(out, input, weight, eps, enable_pdl)
    return out
...
def fused_add_rmsnorm(
    ...
    if enable_pdl is None:
        enable_pdl = is_hopper_arch()
    torch.ops.sgl_kernel_sycl.fused_add_rmsnorm.default(
...
def gemma_rmsnorm(
    ...
    if enable_pdl is None:
        enable_pdl = is_hopper_arch()
    torch.ops.sgl_kernel_sycl.gemma_rmsnorm.default(out, input, weight, eps, enable_pdl)
    return out
...
def gemma_fused_add_rmsnorm(
    ...
    if enable_pdl is None:
        enable_pdl = is_hopper_arch()
    torch.ops.sgl_kernel_sycl.gemma_fused_add_rmsnorm.default(
        input, residual, weight, eps, enable_pdl
    )
...
def gelu_tanh_and_mul(input: torch.Tensor, out: torch.Tensor = None) -> torch.Tensor:
    ...
    torch.ops.sgl_kernel_sycl.gelu_tanh_and_mul.default(out, input, get_cuda_stream())
    return out
...
def gelu_and_mul(input: torch.Tensor, out: torch.Tensor = None) -> torch.Tensor:
    ...
    torch.ops.sgl_kernel_sycl.gelu_and_mul.default(out, input, get_cuda_stream())
    return out
