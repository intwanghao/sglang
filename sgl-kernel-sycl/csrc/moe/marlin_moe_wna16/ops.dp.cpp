...
void permute_cols_kernel(
  ...
  auto read_moe_block_data = [&](int block_id) {
    ...
    sycl::int4* tmp_block_sorted_ids = reinterpret_cast<sycl::int4*>(block_sorted_ids);
    for (int i = 0; i < moe_block_size / 4; i++) {
      tmp_block_sorted_ids[i] = ((sycl::int4*)sorted_token_ids_ptr)[block_id * moe_block_size / 4 + i];
    }
  ...
  auto permute_row = [&](int row) {
    ...
    sycl::half const* a_row_half = reinterpret_cast<sycl::half const*>(a_int4_ptr + in_offset);
    sycl::half* out_half = reinterpret_cast<sycl::half*>(out_int4_ptr + out_offset);
  ...
  for (int index = item_ct1.get_group(2); index < num_moe_blocks; index += item_ct1.get_group_range(2)) {
...
// Auto generated SYCL kernel wrapper used to migration kernel function pointer.
template <int moe_block_size>
void permute_cols_kernel_wrapper(
    const sycl::int4* __restrict a_int4_ptr,
    const int* __restrict perm_int_ptr,
    sycl::int4* __restrict out_int4_ptr,
    const int32_t* __restrict sorted_token_ids_ptr,
    const int32_t* __restrict expert_ids_ptr,
    const int32_t* __restrict num_tokens_past_padded_ptr,
    int size_m,
  ...
  queue.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for(nr, exp_props, [=](sycl::nd_item<3> item_ct1) {
      permute_cols_kernel<moe_block_size>(
          a_int4_ptr,
          perm_int_ptr,
          out_int4_ptr,
          sorted_token_ids_ptr,
...
typedef struct dpct_type_710327 {
  int thread_k;
  int thread_n;
  int num_threads;
} thread_config_t;
...
typedef struct dpct_type_145321 {
  int blocks_per_sm;
...
void marlin_mm(
  ...
  if (has_act_order) {
    // Permute A columns
    auto kernel = permute_cols_kernel_wrapper<8>;
    if (moe_block_size == 8) {
    } else if (moe_block_size == 16)
      kernel = permute_cols_kernel_wrapper<16>;
    else if (moe_block_size == 32)
      kernel = permute_cols_kernel_wrapper<32>;
    else if (moe_block_size == 48)
      kernel = permute_cols_kernel_wrapper<48>;
    else if (moe_block_size == 64)
      kernel = permute_cols_kernel_wrapper<64>;
    else
      TORCH_CHECK(false, "unsupported moe_block_size ", moe_block_size);

    // avoid ">>>" being formatted to "> > >"
    // clang-format off
    dpct::kernel_launcher::launch(kernel, sms, default_threads, 0, stream, A_ptr, perm_ptr, a_tmp_ptr, sorted_token_ids_ptr, expert_ids_ptr, num_tokens_past_padded_ptr, prob_m, prob_k, top_k);
    // clang-format on
    A_ptr = a_tmp_ptr;
    prob_m = prob_m * top_k;
    top_k = 1;
  ...
  */
  // avoid ">>>" being formatted to "> > >"
  // clang-format off
  dpct::kernel_launcher::launch(kernel, blocks, num_threads, max_shared_mem, stream, A_ptr, B_ptr, C_ptr, C_tmp_ptr, s_ptr, zp_ptr, g_idx_ptr, sorted_token_ids_ptr, expert_ids_ptr, num_tokens_past_padded_ptr, topk_weights_ptr, top_k, mul_topk_weights, is_ep, num_groups, prob_m, prob_n, prob_k, locks, use_atomic_add, use_fp32_reduce);
  // clang-format on
}
...
torch::Tensor moe_wna16_marlin_gemm(
  ...
  int sms = -1;
  sms = dpct::get_device(a.get_device()).get_max_compute_units();

  // Alloc buffers
  const c10::OptionalDeviceGuard device_guard(device_of(a));
  ...
  int min_workspace_size = std::min(max_n_tiles * (int)(sorted_token_ids.size(0) / moe_block_size), sms * 4);
  TORCH_CHECK(
  ...
  if (a.scalar_type() == at::ScalarType::Half) {
    MARLIN_NAMESPACE_NAME::marlin_mm<sycl::half>(
        a.data_ptr<at::Half>(),
        b_q_weight.data_ptr(),
        c.data_ptr<at::Half>(),
    ...
    MARLIN_NAMESPACE_NAME::marlin_mm<sycl::ext::oneapi::bfloat16>(
