...
__dpct_inline__ T1 ceil_div(const T1 x, const T2 y) {
  return (x + y - 1) / y;
}

inline std::pair<int, int> GetCudaComputeCapability() {
  int device_id = 0;
  device_id = dpct::get_current_device_id();
  int major = 0, minor = 0;
  major = dpct::get_device(device_id).get_major_version();
  minor = dpct::get_device(device_id).get_minor_version();
...
inline void DebugPrintCUDAArray(T* device_ptr, size_t size, std::string prefix = "") {
  ...
  std::cout << prefix;
  c10::xpu::getCurrentXPUStream().queue().memcpy(host_array.data(), device_ptr, size * sizeof(T)).wait();
 ...
 */
__dpct_inline__ uint32_t sub_if_greater_or_zero(uint32_t x, uint32_t y) {
  return (x > y) ? x - y : 0U;
}

__dpct_inline__ void swap(uint32_t& a, uint32_t& b) {
  uint32_t tmp = a;
  a = b;
  b = tmp;
}

__dpct_inline__ uint32_t dim2_offset(const uint32_t& dim_a, const uint32_t& idx_b, const uint32_t& idx_a) {
  return idx_b * dim_a + idx_a;
}

__dpct_inline__ uint32_t dim3_offset(
...
__dpct_inline__ uint32_t dim4_offset(
