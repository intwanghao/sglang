...
template <typename T, int TPB>
  ...
  auto& normalizing_factor = *sycl::ext::oneapi::group_local_memory_for_overwrite<float>(
      sycl::ext::oneapi::this_work_item::get_work_group<3>());
  auto& float_max = *sycl::ext::oneapi::group_local_memory_for_overwrite<float>(
      sycl::ext::oneapi::this_work_item::get_work_group<3>());
  ...
  for (int ii = item_ct1.get_local_id(2); ii < num_cols; ii += TPB) {
    ...
    threadData = sycl::max(convert_to_float<T>(input[idx]), threadData);
  }

  const float maxElem =
      sycl::reduce_over_group(sycl::ext::oneapi::this_work_item::get_work_group<3>(), threadData, sycl::maximum<>());
  ...
  for (int ii = item_ct1.get_local_id(2); ii < num_cols; ii += TPB) {
    const int idx = thread_row_offset + ii;
    threadData += sycl::exp((convert_to_float<T>(input[idx]) - float_max));
  }

  const auto Z = sycl::reduce_over_group(sycl::ext::oneapi::this_work_item::get_work_group<3>(), threadData, sum);
  ...
  for (int ii = item_ct1.get_local_id(2); ii < num_cols; ii += TPB) {
    const int idx = thread_row_offset + ii;
    const float val = sycl::exp((convert_to_float<T>(input[idx]) - float_max)) * normalizing_factor;
    output[idx] = val;
  }
}
...
void moeTopK(
  ...
  for (int k_idx = 0; k_idx < k; ++k_idx) {
    ...
    cub_kvp inp_kvp;
    for (int expert = item_ct1.get_local_id(2); expert < num_experts; expert += TPB) {
    ...
    /*
    DPCT1118:638: SYCL group functions and algorithms must be encountered in converged control flow. You may need to
    adjust the code.
    */
    /*
    DPCT1065:794: Consider replacing sycl::nd_item::barrier() with
  ...
  if (renormalize && item_ct1.get_local_id(2) == 0) {
...
/*
DPCT1110:639: The total declared local variable size in device function topkGatingSoftmax exceeds 128 bytes and may
...
void topkGatingSoftmax(
  ...
  const int warp_base_row = cta_base_row + item_ct1.get_local_id(1) * ROWS_PER_WARP;
...
#pragma unroll
  for (int ii = 1; ii < VPT; ++ii) {
    thread_max = sycl::max(thread_max, row_chunk[ii]);
  }

// Now, we find the max within the thread group and distribute among the threads. We use a butterfly reduce.
#pragma unroll
  for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
    thread_max = sycl::max(thread_max, SGLANG_SHFL_XOR_SYNC_WIDTH(0xffffffff, thread_max, mask, THREADS_PER_ROW));
  }
...
void topkGatingSoftmaxLauncherHelper(
  ...
  /*
  DPCT1049:641: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
        ...
        stream->submit([&](sycl::handler& cgh) {
          ...
          cgh.parallel_for(
              ...
              [=](sycl::nd_item<3> item_ct1) {
                moeSoftmax<T, TPB>(gating_output, nullptr, softmax_workspace, num_experts);
              });
        });
      }
      {
    ...
    const bool renormalize)  // [num_tokens, num_experts]
{
  ...
  if (dtype == at::ScalarType::Float) {
    ...
    topkGatingSoftmaxKernelLauncher<sycl::half>(
        reinterpret_cast<const sycl::half*>(gating_output.data_ptr<at::Half>()),
        topk_weights.data_ptr<float>(),
        topk_indices.data_ptr<int>(),
        softmax_workspace.data_ptr<float>(),
        num_tokens,
    ...
    topkGatingSoftmaxKernelLauncher<sycl::ext::oneapi::bfloat16>(
        reinterpret_cast<const sycl::ext::oneapi::bfloat16*>(gating_output.data_ptr<at::BFloat16>()),
        topk_weights.data_ptr<float>(),
        topk_indices.data_ptr<int>(),
        softmax_workspace.data_ptr<float>(),
        num_tokens,
