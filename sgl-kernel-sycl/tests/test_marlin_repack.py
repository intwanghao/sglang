...
import pytest
import torch
from sgl_kernel_sycl import awq_marlin_repack
...
def test_awq_marlin_repack_correct(num_bits, k_tiles, n_tiles, group_size):
    ...
    b_weight = torch.randn((size_k, size_n), dtype=torch.float16, device="xpu")
...
if __name__ == "__main__":
    ...
    subprocess.call(["pytest", "--tb=short", "-s", "-v", str(__file__)])
