...
/*
DPCT1049:611: The work-group size passed to the SYCL kernel may exceed the limit. To get the device limit, query
...
#define __CALL_GET_STARTS_KERNEL_BLOCKSCALE(                                                                      \
    ...
    ((sycl::queue*)(stream))->submit([&](sycl::handler& cgh) {                                                    \
      auto a_starts_data_ptr_ct0 = static_cast<ELEMENT_AB_TYPE**>(a_starts.data_ptr());                           \
      auto b_starts_data_ptr_ct1 = static_cast<ELEMENT_AB_TYPE**>(b_starts.data_ptr());                           \
      auto out_starts_data_ptr_ct2 = static_cast<C_TYPE**>(out_starts.data_ptr());                                \
      auto a_scales_starts_data_ptr_ct3 = static_cast<SF_TYPE**>(a_scales_starts.data_ptr());                     \
      auto b_scales_starts_data_ptr_ct4 = static_cast<SF_TYPE**>(b_scales_starts.data_ptr());                     \
      auto alpha_starts_data_ptr_ct5 = static_cast<float**>(alpha_starts.data_ptr());                             \
      auto layout_sfa_data_ptr_ct6 = reinterpret_cast<LayoutSFA*>(layout_sfa.data_ptr());                         \
      auto layout_sfb_data_ptr_ct7 = reinterpret_cast<LayoutSFB*>(layout_sfb.data_ptr());                         \
      auto a_tensors_data_ptr_ct8 = static_cast<ELEMENT_AB_TYPE*>(a_tensors.data_ptr());                          \
      auto b_tensors_data_ptr_ct9 = static_cast<ELEMENT_AB_TYPE*>(b_tensors.data_ptr());                          \
      auto out_tensors_data_ptr_ct10 = static_cast<C_TYPE*>(out_tensors.data_ptr());                              \
      auto a_scales_data_ptr_ct11 = static_cast<SF_TYPE*>(a_scales.data_ptr());                                   \
      auto b_scales_data_ptr_ct12 = static_cast<SF_TYPE*>(b_scales.data_ptr());                                   \
      auto alphas_data_ptr_ct13 = static_cast<float*>(alphas.data_ptr());                                         \
      auto expert_offsets_data_ptr_ct14 = static_cast<int32_t*>(expert_offsets.data_ptr());                       \
      auto sf_offsets_data_ptr_ct15 = static_cast<int32_t*>(sf_offsets.data_ptr());                               \
      auto problem_sizes_data_ptr_ct16 = static_cast<int32_t*>(problem_sizes.data_ptr());                         \
      auto K_ct17 = K;                                                                                            \
      auto N_ct18 = N;                                                                                            \
                                                                                                                  \
      ...
      cgh.parallel_for(                                                                                           \
          ...
          [=](sycl::nd_item<3> item_ct1) {                                                                        \
            __get_group_gemm_starts<ELEMENT_AB_TYPE, C_TYPE, SF_TYPE, float, LayoutSFA, LayoutSFB, ScaleConfig>(  \
                a_starts_data_ptr_ct0,                                                                            \
                b_starts_data_ptr_ct1,                                                                            \
                out_starts_data_ptr_ct2,                                                                          \
                a_scales_starts_data_ptr_ct3,                                                                     \
                b_scales_starts_data_ptr_ct4,                                                                     \
                alpha_starts_data_ptr_ct5,                                                                        \
                layout_sfa_data_ptr_ct6,                                                                          \
                layout_sfb_data_ptr_ct7,                                                                          \
                a_tensors_data_ptr_ct8,                                                                           \
                b_tensors_data_ptr_ct9,                                                                           \
                out_tensors_data_ptr_ct10,                                                                        \
                a_scales_data_ptr_ct11,                                                                           \
                b_scales_data_ptr_ct12,                                                                           \
                alphas_data_ptr_ct13,                                                                             \
                expert_offsets_data_ptr_ct14,                                                                     \
                sf_offsets_data_ptr_ct15,                                                                         \
                problem_sizes_data_ptr_ct16,                                                                      \
                K_ct17,                                                                                           \
                N_ct18);                                                                                          \
          });                                                                                                     \
    });                                                                                                           \
...
void run_get_group_gemm_starts(
  ...
  __CALL_GET_STARTS_KERNEL_BLOCKSCALE(
      ...
      ScaleConfig)
  __CALL_GET_STARTS_KERNEL_BLOCKSCALE(
      cutlass::float_e2m1_t, cutlass::float_ue4m3_t, torch::kFloat16, sycl::half, LayoutSFA, LayoutSFB, ScaleConfig)
  else {
...
void run_fp4_blockwise_scaled_group_mm(
  ...
  hw_info.sm_count = dpct::min(cached_sm_counts[hw_info.device_id], INT_MAX);
