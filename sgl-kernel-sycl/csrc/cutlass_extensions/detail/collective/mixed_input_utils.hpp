...
struct MixedGroupedGemmInputUtils {
 ...
 public:
  ...
  CUTLASS_DEVICE static void lookup_table_convert(
    ...
    {
      sign = dpct::ternary_logic_op(src_reg, 0x88888888, 0x64206420, immLut);
    }
    sign = sign >> 1;
    ...
    if constexpr (KernelConversionMode == ConversionMode::DirectConvert) {
      ...
      if (k_block == 0) {
        ...
        for (int i = 0; i < size(scales_neg_vm_.layout()); ++i) {
          ...
          {
            scale_pos_[0] = dpct::ternary_logic_op(scale_neg_[0], 0xFFFFFF00, 0x80808080, immLut);
            scale_pos_[1] = scale_neg_[1] ^ 0x80808080;
          }
        }
      }
      CUTLASS_PRAGMA_UNROLL
      for (int i = 0; i < size<1>(dst_vm); ++i) {
        lookup_table_convert(
            src_vm(_, i), dst_vm(_, i), scales_neg_vm(_, i), scales_pos_vm(_, i));
      }
  ...
  partition_extra_mma_info(ThreadMma const& mma_thread_slice, TensorStorage& shared_tensors) {
    if constexpr (KernelConversionMode == ConversionMode::DirectConvert) {
      ...
      Tensor tCrS =
          make_tensor<ElementScale>(mma_thread_slice.partition_fragment_A(sS(_, _, Int<0>{})).layout());
