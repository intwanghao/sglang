...
/*
DPCT1110:644: The total declared local variable size in device function gptq_marlin_repack_kernel exceeds 128 bytes and
...
void gptq_marlin_repack_kernel(
  ...
  int start_k_tile = item_ct1.get_group(2) * block_k_tiles;
  ...
  auto wait_for_stage = [&]() {
    ...
    /*
    DPCT1065:799: Consider replacing sycl::nd_item::barrier() with
  ...
  auto load_perm_to_shared = [&](int k_tile_id) {
    ...
    sycl::int4 const* perm_int4_ptr = reinterpret_cast<sycl::int4 const*>(perm_ptr);

    if (item_ct1.get_local_id(2) < perm_size) {
      sh_perm_ptr[item_ct1.get_local_id(2)] = perm_int4_ptr[first_k_int4 + item_ct1.get_local_id(2)];
    }
    /*
    DPCT1065:800: Consider replacing sycl::nd_item::barrier() with
    sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to
    global memory.
    */
    item_ct1.barrier();
  ...
  auto fetch_to_shared = [&](int pipe, int k_tile_id, int n_tile_id) {
    ...
    if constexpr (has_perm) {
      if (item_ct1.get_local_id(2) < stage_size) {
        int k_id = item_ct1.get_local_id(2) / stage_n_threads;
        int n_id = item_ct1.get_local_id(2) % stage_n_threads;
        ...
        cp_async4(
            ...
            reinterpret_cast<sycl::int4 const*>(&(b_q_weight_ptr[src_k_packed * size_n + first_n + (n_id * 4)])));
      }
  ...
  /*
  DPCT1110:643: The total declared local variable size in device function operator() exceeds 128 bytes and may cause
...
/*
DPCT1026:802: The call to cudaFuncSetAttribute was removed because SYCL currently does not support corresponding
setting.
*/
#define CALL_IF(NUM_BITS, HAS_PERM)                                                                                 \
    ...
    {                                                                                                               \
      ...
      stream->submit([&](sycl::handler& cgh) {                                                                      \
        sycl::local_accessor<uint8_t, 1> dpct_local_acc_ct1(sycl::range<1>(max_shared_mem), cgh);                   \
                                                                                                                    \
        auto b_q_weight_ptr_ct0 = b_q_weight_ptr;                                                                   \
        auto perm_ptr_ct1 = perm_ptr;                                                                               \
        auto out_ptr_ct2 = out_ptr;                                                                                 \
        int size_k_ct3 = size_k;                                                                                    \
        int size_n_ct4 = size_n;                                                                                    \
                                                                                                                    \
        ...
        cgh.parallel_for(                                                                                           \
            sycl::nd_range<3>(                                                                                      \
                sycl::range<3>(1, 1, blocks) * sycl::range<3>(1, 1, repack_threads),                                \
                sycl::range<3>(1, 1, repack_threads)),                                                              \
            exp_props,                                                                                              \
...
gptq_marlin_repack(torch::Tensor& b_q_weight, torch::Tensor& perm, int64_t size_k, int64_t size_n, int64_t num_bits) {
  ...
  // Alloc buffers
  const c10::OptionalDeviceGuard device_guard(device_of(b_q_weight));
  ...
  dpct::queue_ptr stream = &c10::xpu::getCurrentXPUStream(dev).queue();
  int blocks;
  blocks = dpct::get_device(dev).get_max_compute_units();

  int max_shared_mem = 0;
  /*
  DPCT1019:801: local_mem_size in SYCL is not a complete equivalent of cudaDevAttrMaxSharedMemoryPerBlockOptin in CUDA.
