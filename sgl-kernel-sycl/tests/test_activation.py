...
@pytest.mark.parametrize("seq_len", [2, 4, 8, 16, 32])
#@pytest.mark.parametrize("batch_size", [2])
#@pytest.mark.parametrize("seq_len", [2])
def test_fused_silu_mul(dim, batch_size, seq_len):
    x = torch.randn(batch_size, seq_len, 2 * dim).to(torch.float16)
    y_ref = x[..., dim:] * torch.nn.functional.silu(x[..., :dim])
    y = torch.empty_like(y_ref, device= "cpu")
    #y_xpu = y.to("xpu")
    x_xpu = x.to("xpu")
    y_xpu = sgl_kernel_sycl.silu_and_mul(x_xpu)
