...
TORCH_LIBRARY_FRAGMENT(sgl_kernel_sycl, m) {
  ...
  m.impl("ep_moe_pre_reorder", torch::kCPU, &ep_moe_pre_reorder);
  m.def(
  ...
  m.impl("ep_moe_silu_and_mul", torch::kCPU, &ep_moe_silu_and_mul);
  m.def(
  ...
  m.impl("ep_moe_post_reorder", torch::kCPU, &ep_moe_post_reorder);
  m.def(
  ...
  m.impl("fp8_blockwise_scaled_grouped_mm", torch::kCPU, &fp8_blockwise_scaled_grouped_mm);
  m.def(
      ...
      "()");
  m.impl("prepare_moe_input", torch::kCPU, &prepare_moe_input);

  m.def("shuffle_rows(Tensor input, Tensor dst2src_map, Tensor output) -> ()");
  m.impl("shuffle_rows", torch::kCPU, &shuffle_rows);
  m.def("apply_shuffle_mul_sum(Tensor input, Tensor output, Tensor permutation, Tensor? factors) -> ()");
  m.impl("apply_shuffle_mul_sum", torch::kCPU, &apply_shuffle_mul_sum);

  m.def("gptq_marlin_repack(Tensor! b_q_weight, Tensor! perm, int size_k, int size_n, int num_bits) -> Tensor");
  m.impl("gptq_marlin_repack", torch::kCPU, &marlin_moe_wna16::gptq_marlin_repack);

  m.def("awq_marlin_repack(Tensor! b_q_weight, int size_k, int size_n, int num_bits) -> Tensor");
  m.impl("awq_marlin_repack", torch::kCPU, &marlin_moe_wna16::awq_marlin_repack);
  ...
  m.def(
      ...
      "                        int n, int k) -> ()");
  m.impl("get_cutlass_w4a8_moe_mm_data", torch::kCPU, &get_cutlass_w4a8_moe_mm_data);
  ...
  m.impl("cutlass_w4a8_moe_mm", torch::kCPU, &cutlass_w4a8_moe_mm);
  ...
  m.impl("moe_wna16_marlin_gemm", torch::kCPU, &moe_wna16_marlin_gemm);
}
