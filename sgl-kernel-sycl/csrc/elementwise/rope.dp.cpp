...
#include <flashinfer/pos_enc.dp.hpp>
...
void apply_rope_pos_ids_cos_sin_cache(
  ...
  /*
  DPCT1009:489: SYCL reports errors using exceptions and does not use error codes. Please replace the
  "get_error_string_dummy(...)" with a real error-handling function.
  */
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(q.scalar_type(), c_type, [&] {
    dpct::err0 status = BatchQKApplyRotaryPosIdsCosSinCache(
    ...
    TORCH_CHECK(
        ...
        "BatchQKApplyRotaryPosIdsCosSinCache failed with error code " +
            std::string(dpct::get_error_string_dummy(status)));
    return true;
  });
  stream->wait();
}
